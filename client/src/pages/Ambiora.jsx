...
const Ambiora = ({ menuData, handleNavigation, formatDate }) => {
  ...
  return (
    <div className="bg-[#F9F0E1] max-w-3xl w-full min-h-[calc(100vh-8rem)] shadow-lg relative overflow-hidden flex flex-col px-4 md:px-8">
      ...
      <div className="flex-1 z-20 relative flex flex-col pt-10">
        ...
        {/* === SNACKS NAVIGATION (TECH ATOM MICRO-INTERACTION) === */}
        <div className={`flex items-center mt-6 z-20 relative ${menuData.type === 'breakfast' ? 'justify-end' : 'justify-between'}`}>
          {menuData.prev && (
            <button onClick={() => handleNavigation(menuData.prev)} className="relative flex items-center group cursor-pointer pl-2 transition-all duration-300 ease-in-out hover:opacity-70">
              ...
              {/* Tech Atom Background */}
              <div className="absolute -left-2 top-1/2 -translate-y-1/2 opacity-20 transition-transform duration-700 group-hover:rotate-180 pointer-events-none">
          ...
          {menuData.next && (
            <button onClick={() => handleNavigation(menuData.next)} className="relative flex items-center group cursor-pointer pr-2 transition-all duration-300 ease-in-out hover:opacity-70">
              ...
              {/* Tech Atom Background */}
              <div className="absolute -right-2 top-1/2 -translate-y-1/2 opacity-20 transition-transform duration-700 group-hover:-rotate-180 pointer-events-none">
                <svg width="60" height="60" viewBox="0 0 100 100" className="stroke-[#2B2B29] fill-none" strokeWidth="2">
                  ...
                  <circle cx="50" cy="50" r="5" className="fill-[#2B2B29]" />
                </svg>
              </div>
            </button>
          )}
