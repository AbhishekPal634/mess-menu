...
import { FaCaretRight, FaCaretLeft, FaExternalLinkAlt } from "react-icons/fa";
import AmbioraPreloader from "../components/AmbioraPreloader";
...
const MenuPage = () => {
  ...
  // Render the preloader if loading or if preloader hasn't finished its minimum display time
  if (loading || !preloaderFinished) {
    return (
      <AmbioraPreloader
        isLoading={loading}
        onFinishedLoading={() => setPreloaderFinished(true)}
      />
    );
  }

  // Remove the banner from error state and update the error UI
  if (error) {
    return (
  ...
  // Update the main return statement to include the banner at the top
  return (
    <div className="min-h-screen bg-[#ECDFCB]">
      {/* Sticky Ad Banner */}
      <div className="sticky top-0 z-50 w-full bg-gradient-to-r from-[#0B1325] to-[#240B36] text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          <a
            className="flex items-center space-x-3"
            href="https://ambioratech.com"
            target="_blank"
          >
            <img
            ...
            />
            <div className="font-tech tracking-wide text-sm text-[#E5E7EB]">
              AMBIORA'25
            </div>
          </a>
          <div className="flex items-center">
            <span className="hidden md:inline mr-2 text-sm font-tech tracking-wide text-gray-400">
              28th FEB - 2nd MAR
            </span>
            <a
              href="https://ambioratech.com/events"
              target="_blank"
              rel="noopener noreferrer"
              className="bg-gradient-to-r from-[#1E3A8A] to-[#5B21B6] hover:from-[#1E40AF] hover:to-[#6D28D9] 
                text-white text-xs font-tech tracking-wide py-1 px-3 rounded flex items-center 
      ...
      {/* Menu Content - Add padding-top to prevent content from going under sticky banner */}
      <div className="flex items-center justify-center p-8 pt-4">
