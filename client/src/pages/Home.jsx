...
const Home = ({ menuData, handleNavigation, formatDate }) => {

  return (
    <div className="bg-[#F9F0E1] max-w-3xl w-full min-h-[calc(100vh-8rem)] shadow-lg relative overflow-hidden flex flex-col px-4 md:px-8">
      
      {/* Unified Decorative Corners */}
      ...
      <div className="flex-1 z-20 relative flex flex-col pt-10">
        ...
        {/* === UNIFIED HEADER: HOME === */}
        <header className="w-full flex flex-col items-center justify-center pb-4 pt-4 min-h-[220px]">
          
          {/* Top Slot: Institutional Sub-header (Takes up same space as the button on other pages) */}
          <div className="h-[32px] flex items-center font-['Cormorant_Garamond'] text-[9px] md:text-[11px] tracking-[0.4em] uppercase text-[#2B2B29]/60 mb-2">
             NMIMS MPSTME <span className="mx-2 font-light">|</span> Shirpur Campus
          </div>

          {/* Main Title: Exact same size and spacing across all pages */}
          <h1 
          ...
          {/* Bottom Slot: Divider */}
          <div className="flex items-center justify-center w-full max-w-[240px] mt-6 opacity-40 h-[20px]">
            <div className="h-[1px] flex-1 bg-[#2B2B29]"></div>
            <div className="w-1.5 h-1.5 rotate-45 border border-[#2B2B29] mx-4"></div>
            <div className="h-[1px] flex-1 bg-[#2B2B29]"></div>
          </div>
        </header>

        {/* === SNACKS NAVIGATION (CLEAN, NO BACKGROUND) === */}
